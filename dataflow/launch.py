...
@app.route('/dataflow-json-conv-on', methods=['POST'])
def conv_token_auth_handler_on():
    ...
    if not auth_token or not _check_token_auth(auth_token):
        ...
        # Dataflow accepts only names consisting of [a-z0-9-]
        # job_name = re.sub('_|:', '-', 'gcs_raw_to_clean_and_bigquery_{}'.format(raw_folder))
        if len(raw_folder) > 0:
            # open_dataflow_cmd = """
            # python3 gcs_raw_to_clean_and_bigquery.py \
            # --job_name={} \
            # --project={} \
            ...
            """.format(
                secrets['bucket_name'], 
                raw_folder, secrets['bucket_name'], raw_folder, 
                secrets['bucket_name'], raw_folder)
