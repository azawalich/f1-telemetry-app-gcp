...
distinct_sessions = '''
select distinct header.sessionUID, insert_time from `f1-telemetry-app.packets_data.car_setup`, UNNEST(header) as header order by insert_time
'''
...
distance_sessions = '''
select distinct header.sessionUID, max(lapData.totalDistance) as totalDistance from `f1-telemetry-app.packets_data.lap`, UNNEST(header) as header, UNNEST(lapData) as lapData GROUP BY header.sessionUID
'''